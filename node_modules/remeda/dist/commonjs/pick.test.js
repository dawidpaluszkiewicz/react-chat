...
describe('data first', function () {
    test('it should pick props', function () {
        var result = (0, pick_1.pick)({ a: 1, b: 2, c: 3, d: 4 }, ['a', 'd']);
        expect(result).toStrictEqual({ a: 1, d: 4 });
    });
    test('allow undefined or null', function () {
        expect((0, pick_1.pick)(undefined, ['foo'])).toEqual({});
        expect((0, pick_1.pick)(null, ['foo'])).toEqual({});
    });
    test('support inherited properties', function () {
        ...
        expect((0, pick_1.pick)(testClass, ['testProp'])).toEqual({ testProp: expect.any(Function) });
    });
});
describe('data last', function () {
    test('it should pick props', function () {
        var result = (0, pipe_1.pipe)({ a: 1, b: 2, c: 3, d: 4 }, (0, pick_1.pick)(['a', 'd']));
        expect(result).toEqual({ a: 1, d: 4 });
    });
});
test('read only', function () {
    (0, concat_1.concat)([1, 2], [3, 4]);
    // or similar:
    ...
    (0, pick_1.pick)(someObject, props); // TS2345 compilation error
});
